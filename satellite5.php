<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
  <title>Welcome to the Satellite 5 Sizing Calculator</title>

<style>

/*!
 * Bootstrap v3.0.0
 *
 * Copyright 2013 Twitter, Inc
 * Licensed under the Apache License v2.0
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Designed and built with all the love in the world @twitter by @mdo and @fat.
 */

.satellte_5 {
  visibility: hidden;
}
.satellite_6 {
  visibility: visible;
}
.logo {
  background-size: cover;
  height: 58px;
  width: 180px;
  margin-top: 6px;
  background-image: url(data:image/svg+xml;base64,);
}
.logo a {
  display: block;
  width: 100%;
  height: 100%;
}
*, *:before, *:after {
  -moz-box-sizing: border-box;
  box-sizing: border-box;
}
aside,
footer,
header,
hgroup,
section{
  display: block;
}
body {
  color: #404040;
  font-family: "Helvetica Neue",Helvetica,"Liberation Sans",Arial,sans-serif;
  font-size: 14px;
  line-height: 1.4;
}

html {
  font-family: sans-serif;
  -ms-text-size-adjust: 100%;
  -webkit-text-size-adjust: 100%;
}
ul {
  margin-top: 0;
}
.container {
  margin-right: auto;
  margin-left: auto;
  padding-left: 15px;
  padding-right: 15px;
}
.container:before,
.container:after {
  content: " ";
  /* 1 */

  display: table;
  /* 2 */
}
.container:after {
  clear: both;
}
.row {
  margin-left: -15px;
  margin-right: -15px;
}
.row:before,
.row:after {
  content: " ";
  /* 1 */

  display: table;
  /* 2 */
}
.row:after {
  clear: both;
}
.col-sm-6, .col-md-6, .col-xs-12 {
  position: relative;
  min-height: 1px;
  padding-left: 15px;
  padding-right: 15px;
}
.col-xs-12 {
  width: 100%;
}

@media print {
  .container {
    width: 670px;
  }
  .col-sm-6 {
    float: left;
  }
  .col-sm-6 {
    width: 90%;
  }
}

@media only screen and (max-device-width: 480px) {
  .container {
    width: 460px;
  }
  .col-sm-6 {
    float: left;
  }
  .col-sm-6 {
    width: 50%;
  }
}

@media (min-width: 768px) {
  .container {
    width: 750px;
  }
  .col-sm-6 {
    float: left;
  }
  .col-sm-6 {
    width: 50%;
  }
}

@media (min-width: 992px) {
  .container {
    width: 970px;
  }
  .col-md-6 {
    float: left;
  }
  .col-md-6 {
    width: 50%;
  }
}

@media (min-width: 1200px) {
  .container {
    width: 1170px;
  }
}

a {
  color: #069;
  text-decoration: none;
}
a:hover {
  color: #EA0011;
  text-decoration: underline;
}
hgroup {
  margin-top: 50px;
}
footer {
  margin: 50px 0 25px;
}
h1, h2, h3 {
  color: #000;
  line-height: 1.38em;
  margin: 1.5em 0 .3em;
}
h1 {
  font-size: 25px;
  font-weight: 300;
  border-bottom: 1px solid #fff;
  margin-bottom: .5em;
}
h1:after {
  content: "";
  display: block;
  width: 100%;
  height: 1px;
  background-color: #ddd;
}
h2 {
  font-size: 19px;
  font-weight: 400;
}
h3 {
  font-size: 15px;
  font-weight: 400;
  margin: 0 0 .3em;
}
p {
  margin: 0 0 2em;
}
p + h2 {
  margin-top: 2em;
}
html {
  background: #f5f5f5;
  height: 100%;
}
code {
  background-color: white;
  border: 1px solid #ccc;
  padding: 1px 5px;
  color: #888;
}
pre {
  display: block;
  padding: 13.333px 20px;
  margin: 0 0 20px;
  font-size: 13px;
  line-height: 1.4;
  background-color: #fff;
  border-left: 2px solid rgba(120,120,120,0.35);
  white-space: pre;
  white-space: pre-wrap;
  word-break: normal;
  word-wrap: break-word;
  overflow: auto;
  font-family: Menlo,Monaco,"Liberation Mono",Consolas,monospace !important;
}

</style>

<script type="text/javascript">

var chart

function calculate()
{
  calculate_database_sizing();
  calculate_cache_sizing();
  calculate_content_sizing();

  if (chart) {
    update_chart()
  }
}

function update_chart()
{
  chart.options.data = [
    {
      type: "stackedBar",
      showInLegend: true,
      name: "Operating System (GiB)",
      axisYType: "secondary",
      color: "#EBA089",
      dataPoints: [
        {y: 20, label: "Satellite Server w/Embedded DB"},
        {y: 20, label: "Satellite Server w/External DB"},
        {y: 20, label: "External Database"},
        {y: 20, label: "Enhanced Reporting Satellite Server"},
        {y: 20, label: "Satellite Proxy"}
      ]
    },
    {
      type: "stackedBar",
      showInLegend: true,
      name: "Swap Space (GiB)",
      axisYType: "secondary",
      color: "#DB9079",
      dataPoints: [
        {y: 8, label: "Satellite Server w/Embedded DB"},
        {y: 8, label: "Satellite Server w/External DB"},
        {y: 8, label: "External Database"},
        {y: 8, label: "Enhanced Reporting Satellite Server"},
        {y: 8, label: "Satellite Proxy"}
      ]
    },
    {
      type: "stackedBar",
      showInLegend: true,
      name: "Application Install (GiB)",
      axisYType: "secondary",
      color: "#8E9F84",
      dataPoints: [
        {y: 5, label: "Satellite Server w/Embedded DB"},
        {y: 5, label: "Satellite Server w/External DB"},
        {y: 5, label: "External Database"},
        {y: 5, label: "Enhanced Reporting Satellite Server"},
        {y: 5, label: "Satellite Proxy"}
      ]
    },
    {
      type: "stackedBar",
      showInLegend: true,
      name: "Database (GiB)",
      axisYType: "secondary",
      color: "#7E8F74",
      dataPoints: [
        {y: parseInt(document.getElementById('database_sizing_requirements').innerHTML), label: "Satellite Server w/Embedded DB"},
        {y: 0, label: "Satellite Server w/External DB"},
        {y: parseInt(document.getElementById('database_sizing_requirements').innerHTML), label: "External Database"},
        {y: parseInt(document.getElementById('database_sizing_requirements').innerHTML), label: "Enhanced Reporting Satellite Server"},
        {y: 0, label: "Satellite Proxy"}
      ]
    },
    {
      type: "stackedBar",
      showInLegend: true,
      name: "Cache (GiB)",
      axisYType: "secondary",
      color: "#F0E6A7",
      dataPoints: [
        {y: parseInt(document.getElementById('cache_sizing_requirements').innerHTML), label: "Satellite Server w/Embedded DB"},
        {y: parseInt(document.getElementById('cache_sizing_requirements').innerHTML), label: "Satellite Server w/External DB"},
        {y: 0, label: "External Database"},
        {y: 0, label: "Enhanced Reporting Satellite Server"},
        {y: parseInt(document.getElementById('cache_sizing_requirements').innerHTML), label: "Satellite Proxy"}
      ]
    },
    {
      type: "stackedBar",
      showInLegend: true,
      name: "Content (GiB)",
      axisYType: "secondary",
      color: "#EBB88A",
      dataPoints: [
        {y: parseInt(document.getElementById('content_sizing_requirements').innerHTML), label: "Satellite Server w/Embedded DB"},
        {y: parseInt(document.getElementById('content_sizing_requirements').innerHTML), label: "Satellite Server w/External DB"},
        {y: 0, label: "External Database"},
        {y: 0, label: "Enhanced Reporting Satellite Server"},
        {y: parseInt(document.getElementById('content_sizing_requirements').innerHTML), label: "Satellite Proxy"}
      ]
    }
  ]
  chart.render();
}

function calculate_database_sizing()
{
    // Set our Sizes (kb)
    var database_sizing_size_per_client = 250;
    var database_sizing_size_per_channel = 500;
    var database_sizing_size_per_package = 230;

    // Capture the entered values of two input boxes
    var database_sizing_num_clients = parseInt(document.getElementById('database_sizing_num_clients').value);
    var database_sizing_num_channels = parseInt(document.getElementById('database_sizing_num_channels').value);
    var database_sizing_avg_packages_per_channel = parseInt(document.getElementById('database_sizing_avg_packages_per_channel').value);

    // Calculate our disk space requirements and display
    var database_sizing_req_clients = database_sizing_num_clients * database_sizing_size_per_client / 1024;
    document.getElementById('database_sizing_req_clients').innerHTML = Math.round(database_sizing_req_clients);

    var database_sizing_req_channels = database_sizing_num_channels * database_sizing_size_per_channel / 1024;
    document.getElementById('database_sizing_req_channels').innerHTML = Math.round(database_sizing_req_channels);

    var database_sizing_req_packages = database_sizing_avg_packages_per_channel * parseInt(database_sizing_num_channels) * database_sizing_size_per_package / 1024;
    document.getElementById('database_sizing_req_packages').innerHTML = Math.round(database_sizing_req_packages);

    // This is the total, in GB
    var database_sizing_requirement = ( database_sizing_req_clients + database_sizing_req_channels + database_sizing_req_packages ) / 1024;
    document.getElementById('database_sizing_requirements').innerHTML = Math.round(database_sizing_requirement);
}

function calculate_cache_sizing()
{
    // Set our Sizes (gb)
    var cache_sizing_size_per_channel = 2;

    // Capture the entered values of two input boxes
    var cache_sizing_num_channels = parseInt(document.getElementById('content_sizing_num_channels').value);

    // Calculate our disk space requirements and display
    var cache_sizing_requirement = cache_sizing_num_channels * cache_sizing_size_per_channel;
    document.getElementById('cache_sizing_requirements').innerHTML = Math.round(cache_sizing_requirement);
}

function calculate_content_sizing()
{
    // Set our Sizes (gb)
    var content_sizing_size_per_channel = 40;

    // Capture the entered values of two input boxes
    var content_sizing_num_channels = parseInt(document.getElementById('content_sizing_num_channels').value);

    // Calculate our disk space requirements and display
    var content_sizing_requirement = content_sizing_num_channels * content_sizing_size_per_channel;
    document.getElementById('content_sizing_requirements').innerHTML = Math.round(content_sizing_requirement);
}

window.onload = function () {
  calculate();

  chart = new CanvasJS.Chart("chartContainer", {
    title:{
      text:"Satellite 5 Storage Requirements"
    },
    animationEnabled: true,
    axisX:{
      interval: 1,
      labelFontSize: 12,
      lineThickness: 0
    },
    axisY2:{
      valueFormatString: "0 GiB",
      lineThickness: 0
    },
    toolTip: {
      shared: true
    },
    legend:{
      verticalAlign: "top",
      horizontalAlign: "center"
    },
  });

  update_chart();
}
</script>
<script type="text/javascript" src="http://canvasjs.com/assets/script/canvasjs.min.js"></script>

</head>
<body>

<section class='container'>

        <hgroup>
          <h1>Welcome to the Satellite 5 Sizing Calculator</h1>
        </hgroup>

        <div class="row"><div id="chartContainer" style="height: 210px; width: 100%;"></div></div>

        <div class="row">
          <div align="center"><h2>Storage Requirements</h2></div>
          <table width="50%" align="center">
            <tr>
              <td>Operating System</td>
              <td>Swap Space</td>
              <td>Application Install</td>
              <td>Database</td>
              <td>Content</td>
              <td>Cache</td>
            </tr>
            <tr>
              <td><b>20 GiB</b></td>
              <td><b>8 GiB</b></td>
              <td><b>5 GiB</b></td>
              <td><b><span id="database_sizing_requirements"></span> GiB</b></td>
              <td><b><span id="content_sizing_requirements"></span> GiB</b></td>
              <td><b><span id="cache_sizing_requirements"></span> GiB</b></td>
            </tr>
          </table>
        </div>

        <div class="row">
          <section class='col-xs-12 col-sm-6 col-md-6'>
            <section>
              <h2>Database Sizing</h2>

                <p>The calculates the space required for the Satellite Database [<a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/sect-Hardware_Requirements.html">1</a>][<a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/General_Database_Requirements.html#Database_Sizing_Requirements">3</a>].  <i>This is required for Satellite Servers w/Embedded Database, Enhanced Reporting Satellite Servers and External PostgreSQL Satellite Databases.</i></p>

                <p>Be generous in database sizing estimates but also consider that size affects the time to conduct backups and adds load to other system resources.  If the database is external, hardware and spacing are entirely dependent on what else is using it.</p>

                <h3>Assumptions</h3>
                <ul>
                  <li>250 KiB per Client</li>
                  <li>500 KiB per Channel</li>
                  <li>230 KiB per Package</li>
                </ul>

                <h3>Mount Points</h3>
                <ul>
                  <li>/opt/rh/postgresql92/root/var/lib/pgsql (versions &gt;= 5.7)</li>
                  <li>/var/lib/pgsql (versions &lt;= 5.6)</li>
                </ul>

                <h3>Calculate</h3>
                <table>
                  <tr><td>Number of Clients</td><td><input type="text" id="database_sizing_num_clients" name="database_sizing_num_clients" value="2000" /> <span id="database_sizing_req_clients" name="database_sizing_req_clients"></span> MiB</td></tr>
                  <tr><td>Number of Channels</td><td><input type="text" id="database_sizing_num_channels" name="database_sizing_num_channels" value="25" /> <span id="database_sizing_req_channels" name="database_sizing_req_channels"></span> MiB</td></tr>
                  <tr><td>Average Package per Channel</td><td><input type="text" id="database_sizing_avg_packages_per_channel" name="database_sizing_avg_packages_per_channel" value="7500" /> <span id="database_sizing_req_packages" name="database_sizing_req_packages"></span> MiB</td></tr>
                  <tr><td></td><td align="center"><input type="button" name="Calculate" value="Calculate" onclick="calculate();" /></td></tr>
                </table>

                <h3>Notes</h3>
                <ul>
                  <li>Recommended: Solid State Drive (SSD) or alternatively a SCSI drive connected to a level 5 RAID.</li>
                  <li>Ensure block sizes are a minimum of 8 KB for Red Hat Satellite to install properly.</li>
                  <li>Ensure the database partition contains an amount of free space equal to the tablespace size.</li>
                </ul>
            </section>
          </section>

          <section class="col-xs-12 col-sm-6 col-md-6">
            <section>
              <h2>Content Sizing</h2>

                <p>This calculates the space required for content, such as packages and configurations [<a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/sect-Hardware_Requirements.html">1</a>][<a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Proxy_Installation_Guide/sect-Hardware_Requirements.html">2</a>].  <i>This is required for Satellite Servers and Proxies.</i></p>

                <p>Channel size estimates assume all layered products are managed; actual usage will vary upon desired channels and architectures.  Backup of this content is optional, but recommended to reduce rebuild times due to download speeds.</p>

                <h3>Assumptions</h3>
                <ul>
                  <li>40 GiB per Channel</li>
                  <li>RHEL 5 (32 & 64 bit)</li>
                  <li>RHEL 6 (32 & 64 bit)</li>
                  <li>RHEL 7 (64 bit only)</li>
                </ul>

                <h3>Mount Points</h3>
                <ul>
                  <li>/var/satellite (for Satellite Servers)</li>
                  <li>/var/spool/rhn-proxy (for Satellite Proxies)</li>
                </ul>

                <h3>Calculate</h3>
                <table>
                  <tr><td>Number of Channels</td><td><input type="text" id="content_sizing_num_channels" name="content_sizing_num_channels" value="5" /></td></tr>
                  <tr><td></td><td align="center"><input type="button" name="Calculate" value="Calculate" onclick="calculate();" /></td></tr>
                </table>

                <h3>Notes</h3>
                <ul>
                  <li>Strongly Recommended: A SCSI drive connected to a level 5 RAID.</li>
                  <li>Ensure block sizes are a minimum of 8 KB for Red Hat Satellite to install properly.</li>
                  <li>For Satellite Proxies, this space is only required to pre-cache and/or store 100% of the RPMs.  The proxy will expire the oldest content based on the amount of space available.</li>
                </ul>
            </section>
          </section>
        </div>

        <div class="row">
          <section class="col-xs-12 col-sm-6 col-md-6">
            <section>
              <h2>Cache Sizing</h2>

                <p>This calculates the space required for caching, such as repository manifests and package lists [<a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/Caching3.html">4</a>].  <i>This is required for Satellite Servers and Proxies.</i></p>

                <h3>Assumptions</h3>
                <ul>
                  <li>2 GiB per Channel</li>
                  <li>RHEL 5 (32 & 64 bit)</li>
                  <li>RHEL 6 (32 & 64 bit)</li>
                  <li>RHEL 7 (64 bit only)</li>
                </ul>

                <h3>Mount Points</h3>
                <ul>
                  <li>/var/cache/rhn (for Satellite Servers)</li>
                  <li>/var/spool/squid (for Satellite Proxies)</li>
                </ul>

                <h3>Notes</h3>
                <ul>
                  <li>Strongly Recommended: A SCSI drive connected to a level 5 RAID.</li>
                  <li>Ensure block sizes are a minimum of 8 KB for Red Hat Satellite to install properly.</li>
                </ul>
            </section>
          </section>

          <section class="col-xs-12 col-sm-6 col-md-6">
            <section>
              <h2>References</h2>
              <ol>
                <li><a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/sect-Hardware_Requirements.html">https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/sect-Hardware_Requirements.html</a></li>
                <li><a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Proxy_Installation_Guide/sect-Hardware_Requirements.html">https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Proxy_Installation_Guide/sect-Hardware_Requirements.html</a></li>
                <li><a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/General_Database_Requirements.html#Database_Sizing_Requirements">https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/General_Database_Requirements.html#Database_Sizing_Requirements</a></li>
                <li><a href="https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/Caching3.html">https://access.redhat.com/documentation/en-US/Red_Hat_Satellite/5.7/html/Installation_Guide/Caching3.html</a></li>
              </ol>
            </section>
          </section>
        </div>


        <footer>
          <div class="logo"><a href="https://www.openshift.com/"></a></div>
        </footer>

</section>

</body>
</html>

